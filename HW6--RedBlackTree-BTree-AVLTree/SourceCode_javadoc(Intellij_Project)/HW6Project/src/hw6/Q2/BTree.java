...
public class BTree<E extends Comparable<E>>  implements SearchTree<E> {
    ...
    /**
     * BTree belirtilen Node'a ait data arrayi uzerinde item bilgisinin olup olmadigi
     * arama islemi ile belirlenir.
     * @param item Arama islemi gerekli olan deger.
     * @param data Uzerinde arama yapılacak BTree deki data arrayi.
     * @param beginValue Arama isleminin baslama yeri.
     * @param endValue Arama isleminin bitis yeri.
     * @return Arama islemi sonrasi item, data arrayi icinde varsa indeks degeri, 
     * yoksa agacin sonunda bulunan bos indeks degeri dondurulur.
     */
    private int binarySearch(E item, E[] data, int beginValue, int endValue) {
        
        if (item.compareTo(data[endValue-1]) > 0)
            return endValue;
        else if (item.compareTo(data[beginValue]) <= 0)
            return beginValue;
        int middleValue;
        if((beginValue + endValue)%2 == 0)
            middleValue = ( beginValue + endValue ) / 2;
        else
            middleValue = ( beginValue + endValue ) / 2 + 1;
        if (middleValue >= endValue) 
            return middleValue;
        if (item.compareTo(data[middleValue]) == 0)
            return middleValue;
        else if (item.compareTo(data[middleValue]) > 0) 
            return binarySearch(item,data,middleValue,endValue);
        else
            return binarySearch(item,data,beginValue,middleValue);
